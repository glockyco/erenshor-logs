<div class="bg-stone-800 border-2 border-stone-700 rounded-lg shadow-lg">
  <div class="border-b border-stone-700 px-6 py-4">
    <div class="flex items-center justify-between">
      <h3 class="font-fantasy text-lg font-semibold text-amber-500">Session Comparison</h3>
      <span
        class="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded border bg-stone-900 text-stone-500 border-stone-700"
      >
        Coming Soon
      </span>
    </div>
  </div>
  <div class="p-6">
    <div class="py-12 text-center">
      <p class="text-sm text-stone-500">
        Compare two sessions side-by-side with delta highlighting
      </p>
      <p class="mt-2 text-xs text-stone-600">Feature planned for Issue #89</p>
    </div>
  </div>
</div>
